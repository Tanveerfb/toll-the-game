...
| **The adverb and an explicit number never appear together.** An off-scale value keeps the verb and states its figure — `raises ATK by 33%` — so "greatly raises DEF by 59%" puts a word meaning 50 in front of a 59. The older `Increases/Decreases … by N%` form is legacy: same meaning, wrong verb | #130 |
| **The brackets in his draft decide what scales — nothing else does.** `[350]%` is ranked; `for 1 turn` written plain is flat, and so is a magnitude with no bracket. Inventing a `valueRanked` for an unbracketed value is the failure that produces every other finding in this row: *"Raises DEF for 1 turn and does [350]% damage"* became a ranked `[25,40,50]` DEF buff, which then made the tier word wrong at both ends. **A tier word and a ranked value are mutually exclusive** — a tier word means flat and unstated (*"Raises ATK and DEF for 2 turns"*), a `[buff.value]%` placeholder means shown and neutral (*"raises all allies' ATK by [buff.value]%"*). **Since #130 there is no sanctioned way to vary the word by rank** — a ladder states its number at every rank instead. The conditional that used to do it (`[debuff? greatly lowers : lowers]`) needed a `ranks` array mirroring the ladder by hand, and `damagePreview` reads that same array as "inactive at this rank", so the mirror going stale silently hid an effect the engine was applying. **When a draft and the JSON disagree, re-read the draft — do not repair the JSON from memory** | #1, #26 |
