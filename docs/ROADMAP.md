...
4. **Gacha** — summon/banners/pity/ult-level dupes/currencies. Spec: `docs/design/GACHA_DESIGN.md`. **UX reference (2026-07-24, 4 screenshots — see memory `toll-gacha-design`, `docs/design/references/gacha-refs/`):** 7DSGC's banner screen (pity-milestone bar with reward icons, paid banner 600-cap/150-step/+30-per-multi vs a free-ticket banner with uneven 60/120/150/180/240 milestones), a "Rates" transparency modal (featured units get a green "Rate Up!" badge vs flat-rate pool units), and a multi-tier coin shop (gold/silver/bronze = F2P, rainbow/pink gems = premium, per-item weekly/daily purchase caps). **Open conflict, needs Tanveer's call before implementation:** the reference's 600-cap paid-banner pity doesn't match this doc's existing locked pity numbers (hard 80, soft 70) — unclear if it supersedes them or is a different, additional mechanic.
5. **Patch-notes system + first patch notes** — in-game changelog + the ~3-week update cadence begins. **UX reference (2026-07-24): Dokkan's in-game News panel** — tabbed categories (Campaign/Event/Other), a scrollable feed of news cards (badge tag + title + banner image + timestamp), each opening a detail view with the full banner, body text, an "Updated on <date>" header, and sub-sectioned content (New Event/New Nodes/New Stage, each with its own banner + link + availability text). Worth adapting this structure rather than a flat changelog text dump.
