...
- **Dokkan wording (rulings #26–27)** — descriptions drop numbers ("raises" <50%, "greatly" 50–79%, "massively" 80%+; same for lowers); pills span the stat ("Raises ATK") and hover shows that skill's exact percentages per rank. One pill per unique effect — cancel effects are phrase-level keys. No "own"; no duration = permanent; cancellable/stackable is the unmentioned default.
...
| 17 | Gon/Killua ult stat raises are `uncancellable` in data but their descriptions no longer say so — under ruling #26's default that reads as cancellable. Tanveer deciding: add "(cannot be cancelled)" to the text, or make the buffs cancellable | `data/characters/gon.json`, `killua.json` | Pending Tanveer |
