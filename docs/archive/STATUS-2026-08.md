...
Session logs folded out of `docs/STATUS.md` on 2026-08-20, when the live doc had
reached 2,733 lines and most of it described systems that no longer exist.

**Moved verbatim.** Nothing here was summarised, trimmed or rewritten on the way
across — the fold is relocation, and an archive that paraphrases has destroyed
the evidence it exists to keep.

Read this for *why* something was built the way it was. For what is true **now**,
read `docs/STATUS.md`.

> **Much of what follows is retired.** Story mode v1 (parts, chapters, the route
> board, orbs, tiles) was deleted wholesale on 2026-08-18 in `8b56767`; ruling
> #108 in `docs/HANDOFF.md` records what replaced it. Entries below that describe
> boards, parts or route tiles are history, not instructions.
